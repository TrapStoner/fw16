...
<Aside type="danger" title="Before you begin">
	This guide requires **Secure Boot disabled** and a **non-encrypted (no LUKS)
	filesystem**. If either applies to you, additional steps are needed that are
	not covered here. If you know what you're doing, you can disable Secure Boot
	in your BIOS — but be aware it removes firmware-level verification of your
	bootloader and kernel, weakening protection against bootkits and unsigned
	code. I am not responsible for any consequences. With that out of the way —
	enjoy!
</Aside>

<Aside type="note" title="Desktop environment">
	This guide is written for and tested on KDE Plasma on Wayland. Steps involving
	power management settings reference KDE System Settings. If you use GNOME,
	sway, or another DE/WM, all kernel and system steps are identical — only the
	DE-specific power management configuration differs. Look up the equivalent
...
<Aside type="caution" title="s2idle only">
	The Framework 16 AMD only supports `s2idle` (suspend-to-idle / S0ix), not S3
	deep sleep. This is a firmware limitation, not an OS one. Hibernate itself
	(S4) works fine and is what this guide sets up.
</Aside>
...
<Aside type="note">
	Btrfs COW and compression are incompatible with swapfiles — the kernel
	requires swapfiles to have contiguous, non-compressed extents. A dedicated
	subvolume lets you disable these without affecting the rest of the filesystem.
</Aside>
...
sudo lsattr /var/swap/swapfile | cut -c1-20

# Should return empty = no compression (correct)
sudo btrfs property get /var/swap/swapfile compression

# Check subvolume flags
sudo btrfs subvolume show /var/swap | grep -i "compress\|flags"
```
...
<table>
  <thead>
    <tr>
      <th>RAM</th>
      <th>zram</th>
      <th>Safe (recommended)</th>
      <th>Minimum</th>
    </tr>
  </thead>
  <tbody>
    <tr><td>32 GB</td><td>8 GB</td><td>40 GB</td><td>24 GB</td></tr>
    <tr><td>64 GB</td><td>8 GB</td><td>72 GB</td><td>56 GB</td></tr>
    <tr><td>64 GB</td><td>16 GB</td><td>80 GB</td><td>48 GB</td></tr>
  </tbody>
</table>
...
<Aside type="tip">
	If you consistently have lots of free RAM when hibernating, the minimum
	formula can work for you however the safe formula is recommended regardless —
	**disk space is cheaper than a corrupted resume**.
</Aside>
...
<Aside type="caution" title="Never use filefrag for btrfs">
	Never use the `filefrag` method for btrfs. Unlike ext4/xfs, btrfs has its own
	internal extent layout. The `btrfs inspect-internal map-swapfile -r` command
	gives the correct physical offset directly with no math required. Using
	`filefrag` will give you a wrong offset and a broken hibernate.
</Aside>
...
<Aside type="note">
	`pri=0` sets the swapfile's priority as low as possible. zram runs at priority
	100 and is always used first. The swapfile is only written to during
	hibernation or if zram is completely exhausted — not as everyday swap. If you
	already have the swapfile in fstab without `pri=0`, add it — the system won't
	break without it but it's better practice.
</Aside>
...
<Aside type="caution">
	Without both parameters, the kernel either cannot find the hibernate image at
	all or reads from the wrong location. `resume=UUID=...` identifies the block
	device; `resume_offset=...` identifies where within it the swapfile begins.
</Aside>

<Aside type="note">
	Parameters are inherited automatically on kernel updates. You do not need to
	rerun this when a new kernel is installed.
</Aside>
...
<Aside type="note">
	Fedora's kernel-install hook rebuilds the initramfs automatically when a new
	kernel is installed. You do not need to rerun dracut on kernel updates.
</Aside>
...
<Aside type="note">
	This is a Fedora-specific step. Distros without SELinux enforcing (Arch,
	Ubuntu default, etc.) can skip it. Without it, hibernate silently fails or
...
<Aside type="caution">
	A clean reboot is required so the audit log is fresh before the SELinux policy
	steps. Leftover audit events from normal use will pollute the generated
	policy.
</Aside>
...
<Aside type="caution">
	Only proceed with generating the policy if the output contains rules involving
	`systemd_logind_t`, `systemd_sleep_t`, or `swapfile_t` exclusively. If you see
	anything else, reboot and redo this step (yes, you have to reboot again) with
	a clean audit log.
</Aside>
...
<Aside type="caution">
	Always trust `audit2allow -b` output **over any guide, including this one** —
	hibernate-related rules will always involve `systemd_logind_t`,
	`systemd_sleep_t`, or `swapfile_t`. If you see rules involving anything else,
	reboot and redo this step (yes, you have to reboot again) with a clean audit
	log.
</Aside>

<Aside type="note">
	The rule changed between Fedora 40 and later versions. On Fedora 41+ the base
	SELinux policy already grants `sys_admin` capability to `systemd_sleep_t` so
	it no longer appears as a denial — instead a directory search permission on
	the swapfile path is what's missing. Future Fedora versions may differ
	further.
</Aside>
...
<table>
  ...
  <tbody>
    <tr><td>AMD RZ616 (MediaTek MT7922)</td><td><code>mt7921e</code></td></tr>
    <tr><td>Intel Wi-Fi 6E AX210</td><td><code>iwlwifi</code></td></tr>
    <tr><td>Intel Wi-Fi 6 AX200</td><td><code>iwlwifi</code></td></tr>
  </tbody>
</table>
...
<Aside type="note">
	The script only triggers on `hibernate` and `suspend-then-hibernate` — not
	regular suspend. WiFi is unaffected during normal sleep/wake cycles.
</Aside>
...
<Aside type="danger">
	Hibernating with a stale `resume_offset` after a partition or swapfile change
	will result in data corruption or a broken resume. Always re-derive the offset
	before the next hibernate.
</Aside>
...
<Aside type="note">
	If you shrink a Windows partition and expand your btrfs partition, the btrfs
	physical extent layout shifts even if you don't touch the swapfile. The offset
	becomes invalid. This applies to Clonezilla backup → repartition → restore
	workflows too — always re-derive the offset after any partition boundary
	change before hibernating.
</Aside>
...
<Aside type="note">
	Only relevant if you have Snapper set up for btrfs snapshots. Snapper is not
	installed by default on Fedora KDE. If you didn't set it up, skip this
	section.
</Aside>
